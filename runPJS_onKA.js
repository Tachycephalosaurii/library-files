/*
    ...
    You can now use sketch(...options) to run code from external programs at the same time.
        sketch({...}, {...}, ...);
...
new Processing(${canvas}.canvas, function(processingInstance) {
    ...
    processingInstance[["KAInfiniteLoopSetTimeout"]] = window.KAInfiniteLoopSetTimeout = processingInstance.KAInfiniteLoopSetTimeout = processingInstance.PJSCodeInjector = window.LoopProtector = 
...
function sketch(...sketches) {
    if (!window.$) alert("Please import jQuery from cdnjs.");
    ...
    img.width = options.canvas.width = options.width || 400;
    img.height = options.canvas.height = options.height || 400;

    /* Fake the canvas */
    img.style.cssText = getCssText(options.canvas);
    img.src = options.url.replace('/cs/', '/computer-programming/') + '/latest.png';

    /* Add the fake behind the real one */
    options.canvas.parentNode.insertBefore(img, options.canvas.nextSibling);

    /* Hide the real canvas */
    options.canvas.style.display = 'none';

    /* Load the canvas */
    var url = 'https://www.khanacademy.org/api/internal/scratchpads/' + options.url.split('/')[5] + '?callback=?';
    $.getJSON(url, function(data) {

        /* run the code */
        options.instance = createPJS(data.revision.code, options);

        /* get rid of that initial flash/delay when loading */
        var id = setInterval(function() {
            options.canvas.parentElement.replaceChild(options.canvas, img);
            options.canvas.style.display = 'inline';
            if (options.onready) options.onready();
            clearInterval(id);
        }, options['load-time'] || 350);
    });
